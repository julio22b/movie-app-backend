...
const add_movie_to_diary = async (req: Request, res: Response): Promise<void> => {
    const movie = await Movie.findOneAndUpdate({ _id: req.params.movieID }, { $inc: { watches: 1 } }, { new: true });
    await User.findOneAndUpdate({ _id: req.params.userID }, { $addToSet: { watched_movies: req.params.movieID } });
    if (movie) {
...
const add_movie_to_watch_list = async (req: Request, res: Response): Promise<void> => {
    ...
    await User.findOneAndUpdate({ _id: req.params.userID }, { $addToSet: { watch_list: req.params.movieID } });
...
const edit_profile = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const file = await cloudinary.uploader.upload(profile_picture as string);

        const update: Partial<IUser> = {
            bio: bio || '',
            username,
            favorites,
            profile_picture: file.secure_url,
        };
...
const add_follower = async (req: Request, res: Response): Promise<void> => {
    ...
    await User.findOneAndUpdate({ _id: follower._id }, { $addToSet: { following: req.params.followedUserID } });
    await User.findOneAndUpdate({ _id: followedUser._id }, { $addToSet: { followers: req.params.followerID } });
